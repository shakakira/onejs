...
function renderLibrary(options, callback){
  ...
  if(options.node){
    return render({ 'template':'library.js', 'view':{}, 'partials':{ 'node_path':'node.path' } }, callback);
  }
...
function renderNode(callback){
  logging.trace('Rendering NodeJS library...');

  var templates = ['path.js', 'process.js'], 
      view = {
        'node_version':process.version,
        'versions':JSON.stringify(process.versions)
      };

  logging.debug('Following NodeJS modules will be rendered: ', modules);

  juxt.async.apply(null, templates.map(function(el){ return render.bind(null, { 'template':'node/'+el, 'view':view }) }))(function(error, modules){
    if(error){
      return callback(error);
    }

    render({ 'template':'node.js', 'view':{}, 'partials':{ 'path':modules[0], 'process':modules[1] } }, callback);
