...
function renderLibrary(options, callback){
  ...
  renderNode(options, function(error, nodeFragment){
    if(error) return callback(error);
    return render({ 'template':'library.js', 'view':{ node:true }, 'partials':{ 'node':nodeFragment } }, callback);
  });
}
...
function renderNode(options, callback){
  logging.trace('Rendering NodeJS library...');
  ...
  options.node && templates.push('process');

  logging.debug('Following NodeJS modules will be rendered: ', templates);

  juxt.async.apply(null, templates.map(function(el){ return render.bind(null, { 'template':'node/'+el+'.js', 'view':view }) }))(function(error, modules){
    if(error){
      return callback(error);
    }
    ...
    var view = {},
        partials = {};
    for(var i = -1, len=templates.length; ++i<len;){
      partials[ templates[i] ] = modules[i];
      view[ templates[i] ] = true;
    }

    render({ 'template':'node.js', 'view':{}, 'partials':partials }, callback);
