...
function construct(wd, manifest, parentPkgs, options){
  ...
  var i = parentPkgs && parentPkgs.length || 0;
  while( i --> 0 ){
    if( parentPkgs[ i ].pkgDict ){
      pkg.pkgDict = parentPkgs[ i ].pkgDict;
      break;
    }
  }

  ! pkg.pkgDict && ( pkg.pkgDict = {} );
