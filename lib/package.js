var path         = require('path'),
    ...
    id           = require('./templating').idGenerator(),
...
function construct(context, callback){
  ...
  var struct = {
    'id'           : context.hasOwnProperty('id') && context.id != undefined ? context.id : id(),
    ...
    'pkgdict'      : context.parent ? context.parent.pkgdict : {},
    'wd'           : context.wd
  };
...
function main(/* [options], [buildOptions], callback */){
  ...
  var callback     = arguments[ arguments.length - 1 ],
      buildOptions = arguments[ arguments.length - 2 ],
      options = arguments.length > 2 ? arguments[0] : buildOptions,

      manifestPath;

  typeof options == 'string'
    && ( manifestPath = options, options = buildOptions = { 'manifestPath': manifestPath } )
    || ( manifestPath = options.manifestPath );

  logging.trace('Loading package from "%s"', manifestPath);
  ...
  manifest(manifestPath, function(error, manifestObj){
    ...
    construct({ 'manifest': manifestObj, 'wd': path.normalize(path.dirname(manifestPath)), 'parent': options.parent }, function(error, pkgobj){
      ...
      content(pkgobj, options, callback);
