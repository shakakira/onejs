...
function test_loadPkg(callback){
  one.loadPkg('example-project/package.json', undefined, { id:templating.idGenerator(), 'azer':1 }, function(error, pkg){
    ...
    try {
      ...
      assert.equal(pkg.modules[0].filename, 'web.js');
      assert.equal(pkg.modules[1].filename, 'a.js');
      assert.equal(pkg.modules[2].filename, 'b.js');
...
function test_collectModules(callback){
  one.collectModules({ 'name':'example-project', 'dirs':{'lib':'lib'}, 'wd':'example-project/' }, function(error, modules){
    try {
      ...
      assert.equal(modules[0].filename, 'web.js');
      assert.equal(modules[1].filename, 'a.js');
      assert.equal(modules[2].filename, 'b.js');
      callback();
    } catch(exc) {
      callback(exc);
    }
